...
async fn thread_defaults_return_unimplemented() {
    let api = MinimalApi;
    assert_unimplemented("thread.create", api.thread_create(sample_create_thread()).await);
    ...
    assert_unimplemented(
        ...
        api.thread_import_sessions().await,
    );
    assert_unimplemented("thread.fork", api.thread_fork().await);
    assert_unimplemented("thread.archive", api.thread_archive(ThreadId::from("t1")).await);
