...
async fn profiles_round_trip_and_hydrate_across_reopen() {
    ...
    assert!(reopened
        .sessions()
        .connection_key(&listed[0].id)
        .is_some(), "hydrated into the hot cache");
}
...
async fn registry_install_pins_and_an_upstream_release_does_not_mutate_it() {
    ...
    let profile = profiles.iter().find(|p| p.id == "claude-acp").expect("profile");
    assert_eq!(profile.health, tethys_schema::agents::ProviderHealth::NotFound);
    assert!(
        profile.detail.as_deref().is_some_and(|d| d.contains("Node.js")),
...
async fn editing_the_executable_or_switching_on_re_checks_that_provider() {
    ...
    assert!(detail(edited).contains("second-missing-binary"), "path edit");
    ...
    assert!(detail(switched).contains("third-missing-binary"), "toggle on");
}
