...
use tethys_schema::connection::{AcpProtocol, AgentCompat, ConnectionKey, NormalizedCapabilities};
...
async fn git_thread_in_worktree_receives_uncommitted_workspace_root_registry() {
    ...
    assert_eq!(
        servers_plain, servers,
        "plain thread must receive identical server set"
    );
}
...
async fn resume_and_recovery_spawn_carry_workspace_root_servers() {
    ...
    assert_eq!(
        initial, resumed,
        "resumed thread must carry identical workspace-root servers"
    );
...
fn v1_connection_always_keeps_stdio_servers_http_only_with_mcp_capabilities() {
    ...
    assert!(
        !transports.http,
        "http must remain false when not negotiated"
    );
