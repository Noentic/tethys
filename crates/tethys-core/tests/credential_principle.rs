...
async fn run() {
    ...
    core.agent_recheck(Some("orphan".into())).await.expect("recheck");
    let orphan = core
        .agent_profiles_list()
        .await
        .expect("list")
    ...
    assert!(!detail.contains("keychain:tethys"), "the reference is not echoed");
    ...
    core.agent_profiles_delete("audit".into())
        .await
        .expect("delete");
    assert_eq!(secrets.get("profile/audit/API_KEY").expect("keychain"), None);
