...
fn tool_patch(update: &acp2::ToolCallUpdate) -> ToolCallPatch {
    ToolCallPatch {
        title: maybe_string(&update.title),
        kind: maybe_value(&update.kind).map(|kind| json_string(kind).trim_matches('"').to_string()),
        ...
        locations: maybe_value(&update.locations)
            .map(|locations| {
                locations
                    .iter()
                    .map(|location| location.path.0.display().to_string())
                    .collect()
            })
            .unwrap_or_default(),
    }
...
fn stop_reason(reason: &acp2::StopReason) -> StopReason {
    match reason {
        ...
        acp2::StopReason::MaxTurnRequests => StopReason::Other("max_turn_requests".to_string()),
...
pub(crate) fn config_option(option: &acp2::SessionConfigOption) -> ConfigOption {
    ConfigOption {
        ...
        current_value: match &option.kind {
            acp2::SessionConfigKind::Select(select) => select.current_value.to_string(),
            acp2::SessionConfigKind::Boolean(boolean) => boolean.current_value.to_string(),
            acp2::SessionConfigKind::Other(_) => String::new(),
            _ => String::new(),
        },
        values: match &option.kind {
            acp2::SessionConfigKind::Select(select) => match &select.options {
                acp2::SessionConfigSelectOptions::Ungrouped(options) => options
                    .iter()
                    .map(|option| option.value.to_string())
                    .collect(),
                _ => vec![],
            },
            acp2::SessionConfigKind::Boolean(_) => {
                vec!["true".to_string(), "false".to_string()]
            }
            acp2::SessionConfigKind::Other(_) => vec![],
            _ => vec![],
        },
    }
}
