...
>
> **`d0-rc4` — pending re-acceptance.** Unlike `d0-rc3`, `d0-rc4` adds visual decisions on top of the accepted handoff (the `awaiting` ring, `workspace-card-selected`, the State Precedence rules, `stop-control`, action-bar collapse order). The acceptance above stands for `d0-rc3`; the Wave 2 rows below reference `d0-rc4` and need it accepted before those chunks implement against it.
...
Phase 1 is decomposed into 19 chunks organized in four waves, with two small serial trunk PRs (M1.6b, M1.6c) between waves 1 and 2. Each chunk is a unit of work sized for one agent in one git worktree, and owns a disjoint set of paths so that concurrent worktrees merge cleanly. Ownership follows the crate/package split in ARCHITECTURE §3: the boundary between chunks *is* the crate boundary.
