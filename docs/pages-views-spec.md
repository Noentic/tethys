...
- **Turn Inspector & Diff Panel (`360px`, Right)**: Collapsible sidebar housing the plan, turn telemetry, raw JSON payloads, and — where the workspace has git — patch inspection and the checkpoint footer. Collapses to an overlay drawer when the viewport drops below `1100px`. **Overlay mode belongs to the Inspector, not the Stage:** the drawer renders over the Stage at Level 3 with a scrim, opens from the tab strip or the diff-summary pill, and dismisses on `Esc` or scrim click; the Stage itself has no overlay mode (DESIGN.md Shell Structure, `stacking`).
...
- `tool-accordion`: Collapsible execution log keyed by `toolCallId`, on `{semantic.surface-nested}`. Header: tool icon, name, and `status-dot`. Body: the command line in `mono-code`, a collapsible input JSON, and streamed content in a sunken code block (`{semantic.surface-sunken}`) with a cap and a `View full` link to the blob. It shows the tool's `kind` (read/edit/delete/move/search/execute/think/fetch/switch_mode/other, one icon each) and `status` (pending/in_progress/completed/failed, a failure also written as the word `Failed`), the `path:line` locations the call reports, and a `tool-origin-tag` when it did not come from the Provider's built-in tools. A failed call and a call awaiting permission open themselves; otherwise the user's own toggle wins and streaming never resets it. Built from `tool_call` / `tool_call_update` notifications, so partial updates patch the same card in place rather than appending a new one. A footer offers `View diff` / `Restore to before this turn` where the capability exists.
...
- `elicitation-card`: Handles the rarer case of a Provider asking the user for structured input mid-turn (`elicitation/create`) — rendered as a small inline form generated from the requested schema, distinct from a permission request (it collects data; it does not authorize a tool call). Providers that don't declare `elicitation` never render it. A fixed-option property renders as a numbered decision (keys `1`–`9`; ACP's option type is a value and a title with no description, so the card never invents a trade-off line), a multi-property request may page with an `n of m` counter and `Back`, and the three protocol outcomes stay distinct — submit is `accept`, `Skip` is `decline`, dismissing is `cancel`. A URL-mode request shows the URL's host and `Open in browser` and never embeds the page (DESIGN.md `elicitation-card`).
- `provider-popover`: Hosts a request from the Provider that is **neither** a `session/request_permission` (that is `permission-request-card`) **nor** an `elicitation/create` (that is `elicitation-card`) — a vendor-extension notification such as `_kiro.dev/mcp/oauth_request`. Both inline cards sit in the transcript at the point they were raised; an extension notification has no such point, so it renders as a popover anchored to the Provider's `model-selector-pill`, visibly attributed to that Provider. It renders the Provider's own actions in the Provider's order (never a hardcoded pair), traps focus while open, and never opens over another focus trap: a request that arrives while a dialog or sheet is open waits in the Provider's pending list, and the pill shows the pending count. Mounted through `registerProviderSurface`, keyed by Provider id and extension method, so a Provider integration contributes a handler without editing a shared file.
- `provider-artifact`: A Provider-emitted artifact that is neither a diff nor tool-call output, mounted through `registerEntryRenderer` so it sits in the transcript where it was emitted. A collapsed summary row (kind glyph, title, size) expands in place; MVP renders text and images and shows a `provider-capability-notice` for any other kind rather than a blank body. It is not the `diff-viewer`, which stays git-shaped.
- `provider-capability-notice`: The one treatment for "this **Provider** cannot do this" — no resume, no elicitation, an unsupported artifact kind. It is distinct from workspace-capability gating (`no git · no revert`, §0.1), which says the **folder** cannot; the two vary independently. A muted sentence naming the Provider and the missing capability, never an error colour. Surfaces render it rather than inventing their own "unsupported" copy.
...
- `composer` (docked `prompt-card` + `composer-chip` pills, `CMP-01..05`): a `/` Tethys command chip (filename = command name, `{{args}}` fills or appends) is listed apart from `/agent:name` commands the Provider advertises, which pass through unchanged and render a clash as `/agent:name` (the Provider's commands form a separate group under a heading naming the Provider, DESIGN.md `composer-command-group`); a `$` skill chip carries a visible injection-method badge — Tethys adds a plaintext instruction naming the skill and pointing at its `SKILL.md`, and the skill body is never inlined for any agent (CMP‑03); an `@` path chip (FFF `search.files`) sends a plaintext reference to the path, never its contents, with a `path:line` echo where line ranges apply (V1). Prompts typed while a session is running are queued — editable, reorderable, persisted — and sending appends without interrupting the stream.
- `usage-bar`: Small SVG ring in the Action Bar showing this Session's token usage against its context budget, when the Provider reports it. The ring is `used ÷ context window size`, so it needs the window size the Provider reports with usage; a Provider that reports tokens without a window size shows the number only. Hidden when unreported and never estimated — but a reported zero renders `0`, because unreported and zero are different facts.
- `tool-run-group`: One summary row for a run of consecutive tool calls — `Read 3 files · ran 2 commands ›` — while streaming and after, so a 30-call turn is not 30 rows. Expanding lists the member `tool-accordion`s; a run holding a failed call or one awaiting permission opens itself and stays open while a member awaits. The `Tool call density` setting (§5.1) switches it off (`Full`).
- `tool-origin-tag`, `subagent-card`: A skill call is tagged `skill · <name>`, an MCP call `mcp · <server>`, and a subagent call is a `tool-accordion` whose body is the child transcript (entries whose `parent_tool_call_id` is that call), one level of indent, collapsed by default with a one-line roll-up. A child awaiting approval forces the parent open and reaches the approval inbox attributed to the subagent. Origin is data the Provider adapter supplies; the webview never infers it from a tool's title.
- `working-indicator`, `turn-notice`: While a turn runs the stage tail shows `Working · 14s` (and the in-flight tool's title); after an idle threshold it appends `No activity for m:ss`. A turn that ends for any reason other than a normal end replaces it with a `turn-notice`: `refusal` (the refused prompt is left out of the next one, and the entry says so), `max_tokens` / `max_turn_requests` (with `Continue`), `cancelled`, `error` (with `Retry` when retryable), `connection-lost` (with `Reconnect`; the transcript is preserved) and `compaction` (a `Context compacted` divider).
- `message-actions`, `code-block`, `jump-to-latest`, `attachment-chip`: A message toolbar (`Copy`, `Fork from here` under the same capability rule as a session row's `Fork`, `Retry` on the last agent message; `Edit & resend` is not in MVP); fenced code with a language label and a `Copy` button; a `↓ Jump to latest` / `↓ 3 new` pill shown only when the user has scrolled off the tail; and image or file chips in the composer and on messages (an image is refused at attach time for a Provider that did not declare image prompts).
- `composer-config-chip`, `config-option-popover`: The docked composer's lower bar shows a **Model** chip (`model` category) and an **Effort** chip (`thought_level` category) built from the Provider's session config options; each opens a small popover, and a change applies from the next turn (stated in a caption under the control). A Provider that declares no option for a category shows no chip for it. `mode` has one home, the action bar's mode pill, and every other option lives only in the full `session-config-panel` behind the Provider pill — no control is rendered twice.
- `activity-ledger`: An Inspector section, the second index over the same history: a session rollup by kind (`Files read 12 · Commands run 5 · MCP calls 3 · Searches 2 · Fetches 1 · Edits 4`, plus `Skills used` and `Subagents` where origin is known), each row expanding into the individual paths, commands, MCP tools grouped by server and URLs, each linking to its transcript entry. Counts derive from tool-call entries; a fresh session reads `No tool calls yet` rather than hiding the section; a `This turn` toggle narrows the scope.
...
| vendor `_`-prefixed notification | `ProviderExtension` | `provider-popover` | M1.7 scaffold; M1.17 transport | defined by M1.6c |
...
- `provider-accordion`: Inline collapsible configuration panel that slides down under its row with zero layout shift (sibling rows translate, never reflow text). Contains:
  ...
  4. `LoginDialog` trigger: **adapts to whatever the Provider's `initialize` response declares in `authMethods`**, rather than assuming a PTY CLI login is the only path — an env-var-based method renders a key/value form; a URL-based method shows a "Sign in with {Provider} →" link plus a code-confirmation field with a visible countdown (many implementations expire around 300s); a CLI-passthrough method opens the isolated PTY `terminal-sheet` running the vendor's own login command (`claude login`, `codex auth`); an **agent-auth** method — the agent runs its own OAuth, starting a local server and opening the browser itself — shows a waiting state (`Waiting for {Provider} to finish sign-in…`) with a `Cancel` action, no code field and no countdown, because the agent, not Tethys, holds the flow. The ACP Registry admits only agent auth and terminal (CLI-passthrough) auth, so those are the two shapes a registry Provider is expected to use; env-var and URL+code remain for manual profiles. If a Provider declares no `authMethods` at all, this control is hidden entirely — there's nothing to log into. Tethys never reads or caches vendor tokens (G7); forms and sheets are input-only, and closing returns focus to the invoking control. The env-var form is the one place a user types a value of their own (an API key for their own agent's environment, not a token Tethys obtained from a vendor login): the value field is masked, each value is handed once to the host (`agent.env_secret_set`), which writes it to the OS keychain and stores only a `keychain:tethys/profile/<id>/<KEY>` reference in the profile; nothing is retained in the webview, there is no method that reads a value back, and deleting the profile deletes the secrets it owns. References are resolved when the agent is launched, and a reference whose secret is missing stops the launch with a stated reason rather than being passed on as if it were the value. Login always opens the surface the Provider's `authMethods` declare — a `LoginDialog` or the `terminal-sheet` — never an inline row form. Closing either surface immediately re-checks that Provider (see Health re-check triggers above); on a successful re-check the row's dot flips amber → green and the Provider becomes selectable in `model-selector-popover` (§3). The URL+code method's countdown is a numeric `m:ss` readout (DESIGN.md `login-dialog`); at 0 the code field reads `Code expired` and offers `Request new code`.
...
**Layout**: A `sync-grid` matrix mapping configured MCP servers (rows) across connected Providers (columns), scoped by a workspace selector in the page header.

> **Decided — matrix, not list-first.** This is the only two-dimensional surface in Settings; Providers (§5.2) and Skills (§5.3) are flat lists on the same nav rail. It stays a matrix because attachment is genuinely many-to-many, and the alternative considered (a list of servers with an attachment-count badge and a per-server drill-in) was rejected. The four questions that decision had left open are now answered: **(1) Grid container:** DESIGN.md `sync-grid` — sticky header row, a frozen `220px` server column, `132px` minimum column width, and horizontal scroll *inside the grid region* as Providers are added (`+ Add Custom ACP Server` makes the column count unbounded); the page never scrolls horizontally. **(2) Keyboard:** the two-dimensional model in DESIGN.md Accessibility (`Grid roving`) with `grid` / `columnheader` / `rowheader` / `gridcell` roles. **(3) Empty state:** zero rows keeps the header and shows `No MCP servers configured`; zero columns keeps the frozen column and shows `No connected Providers`, which links to §5.2. **(4) Axes:** the axes are servers × Providers, and **the workspace is the page's scope selector, not a third axis** — `mcp.attachments(workspace_id)` returns one grid per workspace, the page shows one at a time, and its rows are the global servers plus that workspace's scoped servers. A server scoped to a different workspace does not appear.
...
- `sync-grid`: the matrix container (DESIGN.md `sync-grid`) — sticky header, frozen server column, horizontal scroll, two empty states, `role=grid`. Cells are `sync-grid-cell`.
...
| **`/settings/mcp`** | Session-Attachment Grid | Grid: `sync-grid`, cells: `sync-grid-cell` | MCP servers attached per `session/new` for ACP-native Providers; vendor-file projection kept only as a compatibility fallback. |
