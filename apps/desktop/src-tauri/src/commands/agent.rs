...
#[tauri::command]
#[specta::specta]
pub async fn agent_profiles_delete(
    state: State<'_, CoreState>,
    id: String,
) -> Result<(), String> {
    state
...
#[tauri::command]
#[specta::specta]
pub async fn agent_stderr(state: State<'_, CoreState>, profile_id: String) -> Result<String, String> {
    state
...
pub async fn agent_recheck(
    ...
    state.agent_recheck(profile_id).await.map_err(|e| e.to_string())
}
